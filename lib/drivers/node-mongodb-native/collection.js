...
function iter(i) {
  NativeCollection.prototype[i] = function() {
    ...
    const shouldTimestamp = debug && typeof debug === 'object' && debug.timestamp;
    const opStartTime = shouldTimestamp ? performance.now() : null;
    ...
    if (debug) {
      if (typeof debug === 'function') {
        ...
        this.$print(_this.name, i, args, color, shell, timestamp, opStartTime);
      }
    }
...
NativeCollection.prototype.$print = function(name, i, args, color, shell, timestamp, opStartTime) {
  let prefix = '';
  if (timestamp && opStartTime != null) {
    const ts = new Date().toISOString();
    const duration = (performance.now() - opStartTime).toFixed(3);
    const base = `${ts} [t=${duration}ms]`;
    prefix = color
      ? `\x1B[0;90m${base}\x1B[0m `
      : `${base} `;
  }
